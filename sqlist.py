...
def SELECT(COLUMNS, FROM, WHERE=None):
  """Similarly to SQL, select and project all rows from a table matching the constraints.
...
def project_columns(table, columns):
  ...
  return map(getter, chain([head], data))
...
def select_rows(table, column_spec):
  ...
  # 3. filter
  return chain([head], filter(matchers, data))


def column_indices(head):
  """Build a dictionary of the column indices."""
  return {head[index] : index for index in range(0, len(head))}
...
def all_curry_bind_matcher(column_matchers):
  return (curry_bind_matcher(col, matcher) for col, matcher in column_matchers.items())
...
def all_as_matcher(column_specs):
  return {col : as_matcher(col, spec) for col, spec in column_specs.items()}


def as_matcher(col, func_or_value):
  col = (col, ) if isinstance(col, str) else col
  if callable(func_or_value):
    argspec = getfullargspec(func_or_value)
